...
outputFolder = 'C:\Users\ScanImage\Documents\Diego_Max\phase_diversity_fluorobead\';
...
aberrations(2).name   = 'astig_3_plus';
aberrations(2).nm     = [
    2  2;  
];
...
aberrations(3).name   = 'astig_5_plus';
aberrations(3).nm     = [
    2  -2;  
];
...
aberrations(4).name   = 'astig_3_minus';
aberrations(4).nm     = [
    2  2;  
];
...
aberrations(5).name   = 'astig_5_minus';
aberrations(5).nm     = [
    2  -2;  
];
...
aberrations(7).name   = 'spher_minus';
aberrations(7).nm     = [
    4 0;  
];
aberrations(7).coeffs = [
    -1 * strength;
];
...
%% ---------------- DEFINE UNDERYING ABERRATION ----------------
underlyingRMSWaves = 0.1;
underlyingRngSeed = 15;     %
rng(underlyingRngSeed)
...
raw_coeffs = 2*rand(size(underlying_nm, 1), 1) - 1;

[Z_underlying, pupil_underlying] = buildZernikeBasisOnSLM( ...
    slm.Width, slm.Height, underlying_nm, [center_x, center_y], radius);

raw_phase_waves = Z_underlying * raw_coeffs;
raw_rms_waves = rmsOverPupil(raw_phase_waves, pupil_underlying);


underlying_coeffs = raw_coeffs * underlyingRMSWaves / raw_rms_waves;

fprintf('Generated random underlying aberration with RMS %.4f waves.\n', ...
    underlyingRMSWaves);
fprintf(underlying_coeffs);

%% ---------------- ALLOW FOR CORRECTIONS TO BE ADDED ----------------

correction = zeros(size(underlying_coeffs));
underlying_coeffs = underlying_coeffs + correction;
...
try
    for k = 1:numel(aberrations)
        ...
        %add the underlying aberrations
        nm = [nm; underlying_nm];
        coeffs = [coeffs; underlying_coeffs];
    ...
    save(fullfile(outputFolder, 'aberration_series_log.mat'), ...
        'aberrations', 'log', 'underlyingRMSWaves', ...
        'underlying_nm', 'underlying_coeffs', 'correction');
...
function val = rmsOverPupil(phase_waves, pupil)
% RMS of a vectorized phase map over the circular pupil, in waves.

    phase_waves = phase_waves(:);
    pupil = pupil(:);
    val = sqrt(mean(phase_waves(pupil).^2));
end