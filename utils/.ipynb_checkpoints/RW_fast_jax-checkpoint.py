...
@jax.jit
def rw_kernel_cached_stack(
    theta, sx, sy, sz, mask,
    a_x, a_y, a_z,
    Ax, Ay,
    phase_aberration_stack,
    alpha, k, f, n, mag, w_0, L_bfp,
    dxy_bfp, N_order, z
):
    """
    Batched version of rw_kernel_cached.

    phase_aberration_stack has shape:
        (N_psf, grid_bfp, grid_bfp)

    Returns:
        I_stack with shape (N_psf, grid_ffp_y, grid_ffp_x), matching rw_kernel_cached output
        for each input phase map.
    """
    ...
    return jax.vmap(one_psf)(phase_aberration_stack)


def rw_fast_jax_cached_stack(
    cache,
    alpha, k, f, n, mag, w_0, L_bfp,
    aberrations,
    N_order,
    ...
    """
    Batched cached JAX version of rw_fast_jax_cached.

    Params:
        aberrations: list/tuple of Aberration objects.

    Returns:
        x_ffp, y_ffp, I_stack

    I_stack[j] is the PSF for aberrations[j].
    """
    ...
    for aberration in aberrations:
        ...
        phase_aberration = z_map(theta, phi)
        phase_aberration = np.where(mask, phase_aberration, 0.0)
        phase_aberration_stack.append(phase_aberration)

    phase_aberration_stack = np.stack(phase_aberration_stack, axis=0)

    I_stack = rw_kernel_cached_stack(
        cache["theta"],
        cache["sx"],
        cache["sy"],
        cache["sz"],
    ...
    return cache["x_ffp"], cache["y_ffp"], I_stack
